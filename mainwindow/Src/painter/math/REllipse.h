...
public:
    ...
    void HDataInputStream(QDataStream & input){
         input>>center>>majorPoint>>ratio>>startParam>>endParam>>reversed;
    }

    void HDataOutputStream(QDataStream & output) const{
          output<<center<<majorPoint<<ratio<<startParam<<endParam<<reversed;
    }
    ...
    RVector getTangentAt(const RVector& pt) const;
