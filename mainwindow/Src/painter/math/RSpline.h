 ...
 * \hasStreamOperator
 */
class  RSpline: public RShape, public RExplodable {
public:
    RSpline();
    RSpline(const RSpline& other);
    RSpline(const QList<RVector>& controlPoints, int degree);
    RSpline(const QList<QPointF>& controlPts, int degree);
    //virtual ~RSpline();
    ...
    void HDataInputStream(QDataStream & input){
        input>>controlPoints>>knotVector>>weights>>fitPoints>>degree>>tangentStart>>tangentEnd>>periodic>>dirty>>updateInProgress;
    }

    void HDataOutputStream(QDataStream & output) const{
        output<<controlPoints<<knotVector<<weights<<fitPoints<<degree<<tangentStart<<tangentEnd<<periodic<<dirty<<updateInProgress;
    }
   ...
   bool HGetA3200Gcode(QString &code, const QStringList&paras, const QTransform& trans,  RLine &endPt) const;
    ...
    RVector getTangentAt(double t) const;
