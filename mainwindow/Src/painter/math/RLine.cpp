...
bool RLine::HGetA3200Gcode(QString &code, const QStringList &paras, const QTransform &trans, RLine &endPt) const
{
    ...
    QPointF spt=trans.map(startPoint.get2DPoint());
    QPointF ept=trans.map(endPoint.get2DPoint());
    ...
    if(RS::isDifferentPoint(endPt.startPoint,spt)){
        ...
        if(qAbs(endPt.getAngle()-getAngle())>oang){
            code.append(MotionCompiler::A3200_WAIT_INPOS());
        }
    }

    code.append(MotionCompiler::A3200_G1(ept));

    endPt.setStartPoint(ept);
    endPt.setAngle(getAngle());
