...
bool RCircle::HGetA3200Gcode(QString &code, const QStringList &paras, const QTransform &trans, RLine &endPt) const
{
    ...
    QPointF spt=QPointF(cpt.x() + radius, cpt.y());
    ...
    if(RS::isDifferentPoint(endPt.startPoint,spt)){
        ...
        if(qAbs(endPt.getAngle()-(rflag?M_PI_2:1.5*M_PI))>oang){
            code.append(MotionCompiler::A3200_WAIT_INPOS());
        }
    }
    code.append(MotionCompiler::A3200_G2_G3(rflag,spt,cpt.x()-spt.x(),cpt.y()-spt.y()));

    endPt.setStartPoint(spt);
    endPt.setAngle(rflag?M_PI_2:1.5*M_PI);
...
RVector RCircle::getStartPoint() const
{
    return getPointAtAngle(0.0);
}

RVector RCircle::getEndPoint() const
{
    return getPointAtAngle(0.0);
}
