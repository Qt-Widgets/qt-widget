...
RPainterPath::RPainterPath(QSharedPointer<RShape> sshap)
{
    addShape(sshap);
}
...
void RPainterPath::addPt(const RPoint &pt)
{
    moveTo(pt.getPosition());
}
...
void RPainterPath::addShape(QSharedPointer<RShape> shape) {
    ...
    if (!ellipse.isNull()) {
        ...
        qreal a=ellipse->getCenter().getDistanceTo(ellipse->getMinorPoint());
        qreal b=a*ellipse->getRatio();
        addEllipse(ellipse->getCenter().get2DPoint(),a,b);

        return;
    }

    QSharedPointer<RPoint> pt = shape.dynamicCast<RPoint>();
    if (!pt.isNull()) {
        addPt(*pt);
        return;
    }
