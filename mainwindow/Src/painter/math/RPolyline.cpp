...
bool RPolyline::HGetA3200Gcode(QString &code, const QStringList &paras, const QTransform &trans, RLine &endPt) const
{
    ...
    auto dddinory=getTransformed(trans).dynamicCast<RPolyline>();

    QPointF spt=dddinory->getStartPoint().get2DPoint();
    QPointF ept=dddinory->getEndPoint().get2DPoint();
    ...
    if(RS::isDifferentPoint(endPt.startPoint,spt)){
        ...
        if(qAbs(endPt.getAngle()-RLine(dddinory->vertices.at(0),dddinory->vertices.at(1)).getAngle())>oang){
            code.append(MotionCompiler::A3200_WAIT_INPOS());
        }
    }

    for (int i=0; i<dddinory->countSegments(); i++) {
        QSharedPointer<RShape> segment = dddinory->getSegmentAt(i);

        QSharedPointer<RLine> line = segment.dynamicCast<RLine>();
        if (!line.isNull()) {
            code.append(MotionCompiler::A3200_G1(line->endPoint.get2DPoint()));
            continue;
        }

        QSharedPointer<RArc> arc = segment.dynamicCast<RArc>();
        if (!arc.isNull()) {
            code.append(MotionCompiler::A3200_G2_G3(arc->reversed,arc->getEndPoint().get2DPoint(),arc->center.x-arc->getStartPoint().x,arc->center.y-arc->getStartPoint().y));
            continue;
        }
    }

//    qint32 sizeee=dddinory->vertices.size();
//    for(qint32 k=1;k<sizeee;++k){
//        code.append(MotionCompiler::A3200_G1(dddinory->vertices.at(k).get2DPoint()));
//        if(k!=sizeee-1){
//            if(qAbs(RLine(dddinory->vertices.at(k-1),dddinory->vertices.at(k)).getAngle()-
//                    RLine(dddinory->vertices.at(k),dddinory->vertices.at(k+1)).getAngle())>oang){
//                code.append(MotionCompiler::A3200_WAIT_INPOS());
//            }
//        }
//    }
    ...
    endPt.setAngle(RLine(dddinory->vertices.at(dddinory->vertices.size()-2),dddinory->vertices.last()).getAngle());
...
bool RPolyline::appendShape(const RShape& shape, bool prepend) {
    ...
    if (shape.getShapeType()==RShape::Spline) {
        ...
        if (spl!=NULL) {
            double tol = /*RSettings::getDoubleValue("Explode/SplineTolerance", 0.01)*/0.01;
    ...
    // append ellipse as polyline approximation:
        ...
        if (elp!=NULL) {
            double seg = /*RSettings::getDoubleValue("Explode/EllipseSegments", 32)*/32;
