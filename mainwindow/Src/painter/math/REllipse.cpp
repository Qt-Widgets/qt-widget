...
bool REllipse::HGetA3200Gcode(QString &code, const QStringList &paras, const QTransform &trans, RLine &endPt) const
{
    qreal step=IniSettings::GetInstance()->getValue(IniSettings::Config_microStep).toDouble();

    qreal acc=/*paras.value(HParaLibController::Cam2_acc).toDouble()*/100;
    qreal vel=/*paras.value(HParaLibController::Cam2_speed).toDouble()*/80;

    //    auto mparas=paras;
    //    mparas[HParaLibController::Cam2_acc]=nullptr;
    //    mparas[HParaLibController::Cam2_speed]=nullptr;
    //    code.append(MotionCompiler::A3200_Paras(mparas));

    qint32 size=qint32(getLength()/step)+1;

    bool flag=getLength()<acc*(vel/acc)*(vel/acc);
    ...
    qreal a=center.getDistanceTo(majorPoint);
    qreal b=a*ratio;


    qreal inory=1.0/size;

    QVector<qreal> divides;
    QVector<qreal> timess;
    QVector<qreal> velss;
    qint32 mflag=0;
    qreal adis=acc*(vel/acc)*(vel/acc)*0.5;
    qreal timy1,timy2,vdds;
    for(qint32 k=0;k<size;++k){
        if(k*ts1*acc>vel&&mflag==0){
            divides<<adis/getLength();
            timy1=(vel-k*ts1*acc)/acc;
            timess<<timy1;
            vdds=(k-1)*ts1*acc;
            velss<<vel;

            divides<<k*inory;
            timy2=(step-0.5*acc*(qPow(vel/acc,2)-qPow((k-1)*ts1,2)))/vel;
            timess<<timy2;
            velss<<vel;
            mflag=1;
        ...
        if(mflag==1&&(getLength()-k*inory)<adis){
            divides<<1-adis/getLength();
            timess<<timy2;
            velss<<vel;
    ...
    for(qint32 k=0;k<divides.size();++k){
        poss<<getPointAt(divides.at(k));
        auto pgf=poss.last();
        qreal x2=pgf.x+getAngleAtPoint(pgf)<180.0?-1.0:1.0;
        RVector pgf2(x2,(b*b)*(a*a-x2*pgf.x)/(a*a*pgf.y));
        RLine liner= RLine(pgf,pgf2);
        liner.rotate(RLine(center,majorPoint).getAngle(),liner.getStartPoint());
        dirs<<liner.getAngle();
    }
    ...
    RVector cpt=/*trans.map(center)*/center;

    RVector p(cpt.x + cos(startParam) * getMajorRadius(),cpt.y + sin(startParam) * getMinorRadius());
    p.rotate(getAngle(), RVector(cpt.x,cpt.y));
    RVector spt=/*trans.map(p)*/getStartPoint();

    code.append(MotionCompiler::A3200_G0(spt.get2DPoint()));
    ...
    qint32 sssize=dirs.size();
    for(qint32 k=0;k<sssize;++k){
        //code.append(MotionCompiler::A3200_PVT(poss.at(k),velss.at(k)*cos(dirs.at(k)),velss.at(k)*sin(dirs.at(k)),timess.at(k)));
        code.append(MotionCompiler::A3200_G0(poss.at(k).get2DPoint()));
    }
...
RVector REllipse::getTangentAt(const RVector &pt) const
{
    return RVector();
}
