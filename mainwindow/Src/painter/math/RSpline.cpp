...
RSpline::RSpline(const QList<QPointF> &controlPts, int degree):
     degree(degree), periodic(false), dirty(true), updateInProgress(false), length(RNANDOUBLE)
{
    for(const auto &k:controlPts){
        controlPoints<<k;
    }
}
...
double getTimeOfStep(double v0,double a,double s){
    return (sqrt(v0*v0+2*a*s)-v0)/a;
}

bool RSpline::HGetA3200Gcode(QString &code, const QStringList &paras, const QTransform &trans,RLine &endPt) const
{

    qreal step=IniSettings::GetInstance()->getValue(IniSettings::Config_microStep).toDouble();
    qreal ostp=IniSettings::GetInstance()->getValue(IniSettings::Config_optimizeLength).toDouble();
    qreal oang=IniSettings::GetInstance()->getValue(IniSettings::Config_optimizeAngle).toDouble();

    qreal acc=paras.value(HParaLibController::Cutter_Cam_acc).toDouble();
    qreal vel=paras.value(HParaLibController::Cutter_Cam_speed).toDouble();


    auto mparas=paras;
    mparas[HParaLibController::Cutter_Cam_acc].clear();
    mparas[HParaLibController::Cutter_Cam_speed].clear();
    code.append(MotionCompiler::A3200_Paras(mparas));


    QVector<RVector> poss;
    QVector<qreal> dirs;
    QVector<qreal> timess;
    QVector<qreal> velss;



    auto mshapedd=getTransformed(trans).dynamicCast<RSpline>();


    QList<RSpline> listSpine = mshapedd->getBezierSegments();

    if(listSpine.isEmpty())
        return true;

    velss<<0.0;
    dirs<<listSpine.first().getTangentAt(0.0).getAngle();

    for(const auto&kline:listSpine){
        qint32 count;
        double sdsd=kline.getLength();
        count=kline.getLength()/step+0.5+1.0;

        step=kline.length/count;

        qreal inory=1.0/count;
        for(qint32 k=1;k<count+1;++k){

            double dangle= kline.getTangentAt(k*inory).getAngle();



            if((velss.last()+RS::PointTolerance)<vel){
                qreal ts1=getTimeOfStep(velss.last(),acc,step*k);
                double vvvf=velss.last()+ts1*acc;
                double mvel=vvvf>vel?vel:vvvf;

                qreal dx=qAbs(mvel*cos(dangle)-velss.last()*cos(dirs.last()));
                qreal dy=qAbs(mvel*sin(dangle)-velss.last()*sin(dirs.last()));
                if(dx/ts1>acc||dy/ts1>acc){
                    timess<<(dx>dy?dx/acc:dy/acc);

                }else{
                    timess<<ts1;
                }
                velss<<(vvvf>vel?vel:vvvf);
            }else{

                qreal dx=qAbs(vel*(cos(dangle)-cos(dirs.last())));
                qreal dy=qAbs(vel*(sin(dangle)-sin(dirs.last())));

                qreal ts2=(dx>dy?dx/acc:dy/acc);
                double fdfinot=0.5*acc*ts2*ts2;
                timess<<(step<fdfinot?ts2:ts2+(step-fdfinot)/vel);

                velss<<vel;
            }

            poss<< kline.getPointAt(k*inory);
            dirs<< dangle;
    ...
    velss.removeFirst();
    qreal mggfa=dirs.takeFirst();

    QPointF spt=mshapedd->getStartPoint().get2DPoint();
    QPointF ept=mshapedd->getEndPoint().get2DPoint();
    ...
    if(RS::isDifferentPoint(endPt.startPoint,spt)){
        ...
        if(qAbs(endPt.getAngle()-mggfa)>oang){
            code.append(MotionCompiler::A3200_WAIT_INPOS());
        }
    }
    qint32 sssize=poss.size();
    for(qint32 k=0;k<sssize;++k){
        code.append(MotionCompiler::A3200_PVT(poss.at(k).get2DPoint(),velss.at(k)*cos(dirs.at(k)),velss.at(k)*sin(dirs.at(k)),timess.at(k)));
        //code.append(MotionCompiler::A3200_G1(poss.at(k)));
    }

    endPt.setStartPoint(ept);
    endPt.setAngle(dirs.last());
...
RVector RSpline::getTangentAt(double t) const
{
    updateInternal();
#ifndef R_NO_OPENNURBS
    ON_3dPoint p = curve.TangentAt(t);
    if (p.IsUnsetPoint()) {
        return RVector::invalid;
    }
    return RVector(p.x, p.y);
