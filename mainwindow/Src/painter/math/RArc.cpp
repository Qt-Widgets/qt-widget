...
bool RArc::HGetA3200Gcode(QString &code, const QStringList &paras, const QTransform &trans, RLine &endPt) const
{
    ...
    qreal jdelay=paras.value(HParaLibController::Cutter_Cam_jumpDelay).toDouble();

    QPointF cpt=trans.map(center.get2DPoint());
    QPointF spt=QPointF(cpt.x() + cos(startAngle) * radius, cpt.y() + sin(startAngle) * radius);
    QPointF ept=QPointF(cpt.x() + cos(endAngle) * radius, cpt.y() + sin(endAngle) * radius);

    qreal starAngle=H_Math::convertAngle(startAngle+(reversed?M_PI_2:-M_PI_2));
    qreal etarAngle=H_Math::convertAngle(endAngle+(reversed?M_PI_2:-M_PI_2));
    ...
    if(RS::isDifferentPoint(endPt.startPoint,spt)){
        ...
        code.append(MotionCompiler::A3200_WAIT_INPOS());
        code.append(MotionCompiler::A3200_DWELL(jdelay));

        code.append(MotionCompiler::A3200_PSO_FIRE_ONE());

        code.append(MotionCompiler::A3200_PSO_ON());
    }else{
        if(qAbs(endPt.getAngle()-starAngle)>oang){
            code.append(MotionCompiler::A3200_WAIT_INPOS());
        }
    }

    code.append(MotionCompiler::A3200_G2_G3(reversed,ept,cpt.x()-spt.x(),cpt.y()-spt.y()));

    endPt.setStartPoint(ept);
    endPt.setAngle(etarAngle);
