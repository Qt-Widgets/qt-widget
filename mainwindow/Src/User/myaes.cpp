...
QByteArray MyAes::Crypt(MyAes::Aes level, MyAes::Mode mode, const QByteArray &rawText,
                                 const QByteArray &key, const QByteArray &iv, MyAes::Padding padding)
{
    return MyAes(level, mode, padding).encode(rawText, key, iv);
}

QByteArray MyAes::Decrypt(MyAes::Aes level, MyAes::Mode mode, const QByteArray &rawText,
                                   const QByteArray &key, const QByteArray &iv, MyAes::Padding padding)
{
     return MyAes(level, mode, padding).decode(rawText, key, iv);
}

QByteArray MyAes::ExpandKey(MyAes::Aes level, MyAes::Mode mode, const QByteArray &key)
{
     return MyAes(level, mode).expandKey(key);
}

QByteArray MyAes::RemovePadding(const QByteArray &rawText, MyAes::Padding padding)
{
...
MyAes::MyAes(Aes level, Mode mode,
                               Padding padding)
...
QByteArray MyAes::getPadding(int currSize, int alignment)
{
...
QByteArray MyAes::expandKey(const QByteArray &key)
{
  int i, k;
...
void MyAes::addRoundKey(const quint8 round, const QByteArray expKey)
{
...
QByteArray MyAes::byteXor(const QByteArray &a, const QByteArray &b)
{
...
QByteArray MyAes::cipher(const QByteArray &expKey, const QByteArray &in)
{
...
QByteArray MyAes::invCipher(const QByteArray &expKey, const QByteArray &in)
{
...
QByteArray MyAes::encode(const QByteArray &rawText, const QByteArray &key, const QByteArray &iv)
{
...
QByteArray MyAes::decode(const QByteArray &rawText, const QByteArray &key, const QByteArray &iv)
{
    if (m_mode >= CBC && (iv.isNull() || iv.size() != m_blocklen))
       return QByteArray();
...
QByteArray MyAes::removePadding(const QByteArray &rawText)
{
    QByteArray ret(rawText);
    switch (m_padding)
    {
